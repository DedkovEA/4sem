...
//Function for visualization - plane wave
const double kx = 1;
const double ky = 2;
const double kz = 0;
const double omega = 1;
double plane_wave(double x, double y, double z, double t) {
    return cos(kx*x + ky*y + kz*z - omega*t);
};
...
public:
    ...
    CalcNode(double x, double y, double z, double smth, double vx, double vy, double vz) : x(x), y(y), z(z), smth(smth), vx(vx), vy(vy), vz(vz)
    {
    }
    ...
    void move(double tau) {
        vx = wy * (z-z_0) - wz * (y-y_0);
        vy = wz * (x-x_0) - wx * (z-z_0);
        vz = wx * (y-y_0) - wy * (x-x_0);
        x += vx * tau;
        y += vy * tau;
        z += vz * tau;
    }
...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) : t(0.0) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            double smth = plane_wave(pointX, pointY, pointZ, 0.0);
    ...
    void doTimeStep(double tau) {
        ...
        for(unsigned int i = 0; i < nodes.size(); i++) {
            nodes[i].move(tau);
            nodes[i].smth = plane_wave(nodes[i].x, nodes[i].y, nodes[i].z, t);
        }
    }
