...
class Edge {
 ...
 public:
  Edge(Point_ptr p1, Point_ptr p2, const Polyhedron* const base) : base(base) {
    if(p1 <= p2) {
      pts.first = p1;
      pts.second = p2;
    } else {
  ...
  bool operator<(const Edge& r) const {
    id_type lp1, lp2, rp1, rp2;
    lp1 = this->pts.first;
    lp2 = this->pts.second;
    rp1 = r.pts.first;
    rp2 = r.pts.second;
    return (lp1 < rp1) || ((lp1 == rp1) && (lp2 < rp2));
  };
  ...
  void add_associated_triangle(Triangle_ptr assoc_tr) {
    if(std::find(assoc_trs.begin(), assoc_trs.end(), assoc_tr) != assoc_trs.end()) {
      std::cout << "WARNING: Already existing triangle added to Edge " <<
...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    surface.split_everything(0.0005);
    surface.fix_useless_verticies();
    surface.orient();
